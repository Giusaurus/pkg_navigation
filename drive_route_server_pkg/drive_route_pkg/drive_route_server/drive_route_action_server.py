...
class DriveRouteActionServer(Node):
    ...
    def _wait_for_odom(self, timeout_s: float) -> bool:
        """Blocks (via short sleeps, not a ROS spin -- this runs on the
        execute_callback's own thread under MultiThreadedExecutor, so the
        node's subscription callback keeps firing on another thread
        concurrently) until a pose is available or the timeout elapses.
        """
    ...
    def execute_callback(self, goal_handle):
        ...
        while wp_idx < len(waypoints):
            ...
            if current_pose is None:
                # Odom dropped out mid-route -- stop rather than drive blind.
                self._publish_stop()
